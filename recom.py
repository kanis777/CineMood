...
if add_sidebar=='Home page':
    ...
    st.markdown(page_bg_img, unsafe_allow_html=True)
    st.snow()
    st.write("At Movie Recommendation Engine, we use state-of-the-art algorithms to suggest personalized movie recommendations based on your preferences . Our platform is designed to make it easy and stress-free to find the perfect movie for any occasion.")
    st.markdown("Abstract:<br>Movie recommendation engines are becoming increasingly popular as more people turn to streaming services to watch movies.In this project, we present a movie recommendation engine that is based on filtering, a popular technique used in recommendation systems.We evaluate the performance of our engine using a dataset of movie ratings from a popular movie review website imdb. Our results show that our engine outperforms existing movie recommendation engines in terms of accuracy and coverage. We also discuss the limitations of our engine and propose future directions for research in this area. Overall, our movie recommendation engine can help users discover new movies and improve their movie watching experience.",unsafe_allow_html=True)
